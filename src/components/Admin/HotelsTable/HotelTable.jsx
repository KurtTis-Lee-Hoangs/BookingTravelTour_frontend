...
const HotelTable = ({ hotels }) => {
  ...
  //       toast.error("Error uploading photo to Cloudinary", error.message);
  //     } finally {
  ...
  const handlePhotoChange = async (e, setHotelState) => {
    ...
    if (file) {
      ...
      // Tạo FormData để gửi file ảnh tới server backend
      const formData = new FormData();
      formData.append("file", file); // Gửi file lên backend
  
      try {
        // Gửi ảnh lên server của bạn (tương tự như handleAvatarChange)
        const response = await fetch(`${BASE_URL}/uploads/admin`, {
          method: "POST",
          body: formData, // Gửi form data tới server
          credentials: "include",
        });
        ...
        if (data.url) {
          // Cập nhật ảnh mới vào state của khách sạn (hotel state)
          setHotelState((prev) => ({
            ...prev,
            photo: data.url, // Add the new photo URL to the state
          }));
          toast.success("Image uploaded successfully!");
        } else {
        ...
        toast.error("Error uploading photo to server");
      } finally {
